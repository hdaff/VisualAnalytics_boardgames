...
export function setupConnection(socket) {
  ...
  socket.on("getRelevantData", obj => {
    let parameters = obj.parameters;
    let data = [];
    fs.createReadStream(file_path + file_name_csv).pipe(parse({
      delimiter: ',',
      columns: true
    })).on('data', row => {
      data.push(row);
  ...
  socket.on("getRelevantGraphData", obj => {
    ...
    fs.createReadStream(file_path + file_name_csv).pipe(parse({
      ...
      const links = [];
      const allIds = nodes.map(n => {
        return n.ID;
      });
      nodes.forEach((n, i) => {
        // console.log("Node", i, ":", n);
        if (n.recommendation1 && allIds.includes(n.recommendation1)) links.push({
          source: n.ID,
          target: n.recommendation1
        });
        if (n.recommendation2 && allIds.includes(n.recommendation2)) links.push({
          source: n.ID,
          target: n.recommendation2
        });
        if (n.recommendation3 && allIds.includes(n.recommendation3)) links.push({
          source: n.ID,
          target: n.recommendation3
        });
        if (n.recommendation4 && allIds.includes(n.recommendation4)) links.push({
          source: n.ID,
          target: n.recommendation4
        });
        if (n.recommendation5 && allIds.includes(n.recommendation5)) links.push({
          source: n.ID,
          target: n.recommendation5
        });
        if (n.recommendation6 && allIds.includes(n.recommendation6)) links.push({
          source: n.ID,
          target: n.recommendation6
        });
      });
