...
export function selectGamesByMechanic(games, mechanic) {
  if (mechanic === 'all') return games;else {
    return games.filter(game => {
...
export function in_top_5_popular_categories(games) {
  const top_5_categories = ['Economic', 'Fantasy', 'Science Fiction', 'Adventure', 'Fighting'];
  games.forEach(game => {
...
export function in_top_5_popular_mechanics(games) {
  let top_5_mechanics = ['Hand Management', 'Solo / Solitaire Game', 'Variable Player Powers', 'Variable Set-up', 'Worker Placement'];
  games.forEach(game => {
